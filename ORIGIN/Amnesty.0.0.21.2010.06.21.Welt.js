alert((
	'°Amnesty.0.0.21.2010.06.21.Welt.js°:'
	+'\n++\t'
	+'\n++\t'
	+'\n++\t'
));
...
//Core/Core.Welt.js
	/*TODO
		...
		//v//var nummer is build via: var nummer=C.Welt.Variable.iZahl+1; -- should be C.Welt.Variable.iZahl++;
	...
	*/
	/*DONE
		...
		//built method °filler°; it fills the tiles on the °minimap°-canvas
		//it seems the minimap is filled twice; first in the °mini°-method and second in the °updateMiniMap°-method
	*/
...
=function()
{
	theMapsBackGround.link().width
	=mapWidth*miniMapScale;

	theMapsBackGround.link().height
	=mapHeight*miniMapScale;

	theMapsPlayGround.link().width
	=theMapsBackGround.link().width;

	theMapsPlayGround.link().height
	=theMapsBackGround.link().height;

	theMapsBackGround.link().style.width
	=theMapsPlayGround.link().style.width
	=this.link().style.width
	=(mapWidth*miniMapScale)+'px';

	theMapsBackGround.link().style.height
	=theMapsPlayGround.link().style.height
	=this.link().style.height
	=(mapHeight*miniMapScale)+'px';

	theMapsBackGround.says().fillStyle
	='transparent';
