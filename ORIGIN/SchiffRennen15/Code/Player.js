Player=function(){
	...
	this.xFuture=this.xActual+Math.cos(this.rot)*this.moveStep;
	this.yFuture=this.yActual+Math.sin(this.rot)*this.moveStep;
	//first make sure that we cannot move outside the boundaries of the level
	if(this.yFuture < 0 || this.yFuture > mapHeight ||this.xFuture < 0 || this.xFuture > mapWidth){
		return true;
	}
	//return true if the map block is not 0,ie. if there is a blocking wall.
	return(map[Math.floor(this.yFuture)][Math.floor(this.xFuture)] > wer &&map[Math.floor(this.yFuture)][Math.floor(this.xFuture)] < (wer+19));
	}
